...
public interface IBienDetalleMantenimientoRepository extends JpaRepository<BienDetalleMantenimientoEntity, Serializable>{
 ...
 // Conteo de la busqueda
    @Query("SELECT COUNT(objDetalleMant) FROM BienDetalleMantenimientoEntity  objDetalleMant WHERE ( objDetalleMant.observaciones like %?1%)")
    public abstract long countForDataTable(String search);

    // Conteo de la busqueda
    @Query("SELECT COUNT(objDetalleMant) FROM BienDetalleMantenimientoEntity objDetalleMant WHERE ( objDetalleMant.observaciones like %?1%) AND CONVERT(objDetalleMant.creacionFecha, DATE) BETWEEN ?2 AND ?3")
    ...
    @Query("SELECT objDetalleMant FROM BienDetalleMantenimientoEntity objDetalleMant WHERE ( objDetalleMant.observaciones like %?1%) AND CONVERT(objDetalleMant.creacionFecha, DATE) BETWEEN ?2 AND ?3 order by objDetalleMant.estatus_recordatorio ASC")
