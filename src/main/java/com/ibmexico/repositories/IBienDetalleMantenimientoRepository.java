...
public interface IBienDetalleMantenimientoRepository extends JpaRepository<BienDetalleMantenimientoEntity, Serializable>{
    ...
    @Query("SELECT objActivo FROM BienDetalleMantenimientoEntity objDetalleMant, BienActivoEntity objActivo  WHERE objDetalleMant.idBienActivo=?1  AND objActivo.idRecursoActivo=?1")
 ...
 // Conteo de la busqueda
    @Query("SELECT COUNT(objDetalleMant) FROM BienDetalleMantenimientoEntity  objDetalleMant WHERE ( objDetalleMant.diagnostico like %?1%)")
    public abstract long countForDataTable(String search);

    // Conteo de la busqueda
    @Query("SELECT COUNT(objDetalleMant) FROM BienDetalleMantenimientoEntity objDetalleMant WHERE ( objDetalleMant.diagnostico like %?1%) AND CONVERT(objDetalleMant.creacionFecha, DATE) BETWEEN ?2 AND ?3")
    ...
    @Query("SELECT objDetalleMant FROM BienDetalleMantenimientoEntity objDetalleMant WHERE ( objDetalleMant.diagnostico like %?1%) AND CONVERT(objDetalleMant.creacionFecha, DATE) BETWEEN ?2 AND ?3 order by objDetalleMant.estatus_recordatorio ASC")
    ...
    @Query("SELECT objDetalleMant FROM BienDetalleMantenimientoEntity objDetalleMant WHERE ( objDetalleMant.diagnostico like %?1%) order by objDetalleMant.estatus_recordatorio ASC")
